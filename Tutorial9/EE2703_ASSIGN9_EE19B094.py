"""
PURPOSE : EE2703 Assignment 9
AUTHOR  : Manvar Nisharg -- EE19B094
INPUT   : None
OUTPUT  : 18 graphs saved in 'plots' folder
"""
...
class General_Plotter():
    ''' Class used for plotting different plots'''
...
def surface_plotter(Y_mags,Y_angles,savename):
    ...
    surf=ax.plot_surface(t,w,Y_mags.T,cmap=plt.get_cmap('jet'),linewidth=0, antialiased=False)
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    surf=ax.plot_surface(t,w,Y_angles.T,cmap=plt.get_cmap('jet'),linewidth=0, antialiased=False)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    plt.ylabel("frequency")
    plt.xlabel("time")
    ax.set_ylim(-80,80)
